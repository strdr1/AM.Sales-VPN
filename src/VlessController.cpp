...
void VlessController::addKey(const QString &vlessUri)
{
    const Server s = parseVless(vlessUri);
    if (!s.valid) {
        emit errorOccurred(QStringLiteral("Не удалось распознать ключ vless://"));
        return;
    }
    m_serversData.append(s);
    if (m_store) m_store->addKey(s.rawUri);   // сохраняем в БД
    rebuildServerList();
