// ─────────────────────────────────────────────────────────────────────────
//  Main.qml — главный экран AM.SALES VPN (Apple-минимализм).
//
...
Window {
    ...
    minimumHeight: 620
    visible: true
    title: qsTr("AM.SALES VPN")
    color: "#070806"

    // Ширина левой панели (можно тянуть за правую границу).
    property real panelW: 340

    readonly property color accent: "#9FE812"
    readonly property color textHi: "#F4F6F0"
    readonly property color textLo: "#7E8A72"

    // ── Красивый шрифт Manrope (variable). Применяем ко всему. ──────────
    FontLoader {
        id: manrope
        source: "qrc:/qt/qml/AmSalesVPN/assets/fonts/Manrope-Regular.ttf"
    }
    ...
    Rectangle {
        anchors.fill: parent
        gradient: Gradient {
            GradientStop { position: 0.0; color: "#08110A" }
            GradientStop { position: 0.5; color: "#060806" }
            GradientStop { position: 1.0; color: "#0A0E08" }
        }
    }

    // Очень деликатное зелёное пятно (верх-лево), еле заметное.
    Rectangle {
        x: -160; y: -120
    ...
    RowLayout {
        ...
        Item {
            ...
            Image {
                id: logo
                source: "qrc:/qt/qml/AmSalesVPN/assets/AmSalesLogoLight.png"
                anchors.top: updateBanner.bottom
                anchors.topMargin: updateBanner.visible ? 12 : 40
